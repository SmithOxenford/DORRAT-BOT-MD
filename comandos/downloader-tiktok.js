...
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: 'BOT-MAKANAKY-GAAA', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/ADRIANOSANCHEZ24/BOT-MAKANAKY-GAAA' }, mentionedJid: [m.sender] }}}, { quoted: m })
...
conn.sendMessage(m.chat, { video: { url: data[data.length - 1] }, caption: 'MAKANAKY-BOT-GAAA', footer: await shortUrl(data[data.length - 1]), buttons }, { quoted: m })
} catch {
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: 'BOT-MAKANAKY-GAAA', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/ADRIANOSANCHEZ24/BOT-MAKANAKY-GAAA' }, mentionedJid: [m.sender] }}}, { quoted: m })
...
await conn.relayMessage(m.chat, prep.message, { messageId: prep.key.id, mentions: [m.sender] })
let buttons = [{ buttonText: { displayText: '𝙰𝚄𝙳𝙸𝙾' }, buttonId: `${usedPrefix}tomp3` }]
conn.sendMessage(m.chat, { video: { url: url}, caption: 'MAKANAKY-BOT-GAAA', footer: await shortUrl(url), buttons }, { quoted: m })
} catch {
